...
from src.backend.core.response import MESSAGE_AUTHENTICATION_ERROR_401, ErrorMessageSchema, MESSAGE_REGISTER_ERROR_401
...
from src.backend.core.exceptions import AuthenticationError, RegisterError
...
class RegisterMethodView(MethodView, HashPWMixin, JWTWithGetTokenMixin):
    ...
    def post(self, register_data: RegisterSchema) -> tuple[Response, int]:
        ...
        try:
            ...
            if user:
                raise RegisterError(f"{MESSAGE_REGISTER_ERROR_401}: Пользователь с таким username уже существует")
        ...
        except (RegisterError, AuthenticationError) as e:
            return jsonify(ErrorMessageSchema(message=str(e)).model_dump()), 401
